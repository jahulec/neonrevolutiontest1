...
export function renderNews({ locale, lang, news }) {
  ...
  const cards = news.map((entry) => {
    ...
    return `<article class="square-card">${tile}<div class="square-card__caption"><h2>${escapeHtml(copy.title)}</h2>${entry.displayDate ? `<time datetime="${escapeHtml(entry.date)}">${escapeHtml(entry.displayDate)}</time>` : ''}</div></article>`;
  }).join('\n');
