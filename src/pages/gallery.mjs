import { escapeHtml } from '../lib/html.mjs';

function localized(value, lang) {
  return typeof value === 'object' ? value[lang] : value;
}

function renderItem(item, index, locale, lang) {
  if (item.status === 'placeholder') {
    return `<div class="gallery-item gallery-item--placeholder" aria-label="${escapeHtml(locale.gallery.placeholder)}">
      <span aria-hidden="true">NR // ${String(index + 1).padStart(2, '0')}</span>
      <strong>${escapeHtml(locale.gallery.soon)}</strong>
    </div>`;
  }

  const title = localized(item.title, lang);
  const description = localized(item.description, lang);
  return `<button class="gallery-item gallery-item--photo" type="button" data-gallery-open data-gallery-image="${escapeHtml(item.image)}" data-gallery-title="${escapeHtml(title)}" data-gallery-description="${escapeHtml(description)}" aria-label="${escapeHtml(`${locale.gallery.open}: ${title}`)}">
    <img src="${escapeHtml(item.image)}" width="${escapeHtml(item.width ?? 1600)}" height="${escapeHtml(item.height ?? 1200)}" alt="${escapeHtml(title)}" loading="lazy" decoding="async">
    <span class="gallery-item__overlay"><strong>${escapeHtml(title)}</strong><small>${escapeHtml(description)}</small></span>
  </button>`;
}

export function renderGallery({ locale, lang, gallery }) {
  const items = gallery.map((item, index) => renderItem(item, index, locale, lang)).join('\n');
  return `<header class="page-masthead page-masthead--plain content"><h1>${escapeHtml(locale.pages.gallery.heading)}</h1></header>
  <section class="page-section content gallery-page" aria-label="${escapeHtml(locale.pages.gallery.heading)}"><div class="gallery-grid">${items}</div></section>
  <dialog class="gallery-lightbox" data-gallery-modal aria-labelledby="gallery-modal-title">
    <button class="modal-close gallery-lightbox__close" type="button" data-modal-close aria-label="${escapeHtml(locale.close)}">×</button>
    <button class="gallery-lightbox__nav gallery-lightbox__nav--prev" type="button" data-gallery-prev aria-label="${escapeHtml(locale.gallery.previous)}">←</button>
    <figure><img data-gallery-image src="" alt=""><figcaption><strong id="gallery-modal-title" data-gallery-title></strong><span data-gallery-description></span></figcaption></figure>
    <button class="gallery-lightbox__nav gallery-lightbox__nav--next" type="button" data-gallery-next aria-label="${escapeHtml(locale.gallery.next)}">→</button>
  </dialog>`;
}
