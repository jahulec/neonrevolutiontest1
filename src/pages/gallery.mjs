...
export function renderGallery({ locale, lang, gallery }) {
  ...
  <dialog class="gallery-lightbox" data-gallery-modal aria-label="${escapeHtml(locale.pages.gallery.heading)}">
    ...
    <figure><img data-gallery-image src="${escapeHtml(initial?.image ?? '/assets/sygnet-neon.webp')}" alt="${escapeHtml(initialAlt)}" loading="lazy" decoding="async" draggable="false"></figure>
